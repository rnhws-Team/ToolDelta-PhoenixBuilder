...
func (o *PickBlock) blockPick(x, y, z int32) {
	...
	if resp.Error != nil {
		panic(fmt.Sprintf("blockPick: %v", resp.Error))
	}
...
func (o *PickBlock) onInvoke(chat *defines.GameChat) bool {
	...
	go func() {
		...
		x, y, z := int32(math.Floor(float64(pos[0]))), int32(math.Floor(float64(pos[1]-1.62001001834869))), int32(math.Floor(float64(pos[2])))
		// 尝试 Pick 方块
		o.blockPick(x, y, z)
		// 面向玩家并尝试丢出方块
